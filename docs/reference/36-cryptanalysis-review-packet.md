...
| C13 | Masking / rounding / Montgomery encoding | `ml_adsa_masking.ec`, `ml_adsa_rounding.ec`, `ml_adsa_montgomery.ec` | — (first principles; integer arithmetic) |
...
**Machine-checked (30 prover artifacts, 153 lemmas = 121 EasyCrypt + 32 Coq, 35/35 genuineness, 6 Gobra;
...
5. **Structural transcription** of the NTT loops to the int32/int64 code (arithmetic kernels proven; the
   loop wiring is byte-validated against CIRCL + go-qrllib, not source-proved).
...
cd formal && zsh check-all.sh          # → ALL GREEN (20 classical + 5 quantum + 5 Coq)
