...
| validation | **CAVP/ACVP** (prerequisite for FIPS 140-3) | **none** (research scheme); base inherits ACVP via CIRCL/go-qrllib; code-level Gobra proofs + 36 algorithm proofs (244 lemmas) + KATs are stronger-than-typical evidence but are **not** CAVP validation |
