...
`formal/check-all.sh` → **26 classical EasyCrypt + 5 quantum (EasyPQC) + 5 Rocq = 36 artifacts**;
`formal/genuineness.sh` → **50/50** (each weakening breaks its proof). Go: `go test ./...` PASS.
...
theorem), `dkey_ll`. Verified: zero `admit/Admitted/sorry`; 50/50 genuineness.
