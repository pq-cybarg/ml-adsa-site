...
`formal/check-all.sh` → **22 classical EasyCrypt + 5 quantum (EasyPQC) + 5 Rocq = 32 artifacts**;
`formal/genuineness.sh` → **41/41** (each weakening breaks its proof). Go: `go test ./...` PASS.
...
theorem), `dkey_ll`. Verified: zero `admit/Admitted/sorry`; 41/41 genuineness.
