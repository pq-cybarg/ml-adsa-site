...
and NTT inversion — are now source-proved from first principles; the *implementation* is byte-anchored to
two independent FIPS-204 references and KAT-pinned. The only thing left un-source-proved is the **bit-level
Montgomery encoding** for the specific ζ=1753 (matching the code's int32 Montgomery representation) — a pure
**implementation-conformance** detail (the code's int32 NTT computes the same ring values the proofs reason
about abstractly over `R_q`), **not a security assumption**; the algorithm-level proofs never invoke the
NTT. It is validated by byte-exact dual-reference equality against CIRCL + go-qrllib.
...
|---|---|---|---|
| EasyCrypt (classical) | opam switch `discus-verified` | 19 files | green |
...
| 7 | EUF-CMA (T1; §8.2) | aggregate + verify | `ml_adsa_euf.ec : msufcma_uncond` (≈ line 137) — EC | — |
