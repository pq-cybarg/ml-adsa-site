...
```
Prover artifacts : 35   (25 classical EasyCrypt + 5 quantum EasyPQC + 5 Coq/Rocq)
Machine-checked lemmas : 234   (202 EasyCrypt + 32 Coq)
Genuineness checks : 49/49
...
### 3.1 Stale / contradictory counts (canonical-current = 35 artifacts / 234 lemmas / 49 genuineness / 6 Gobra)

> The per-line `→` targets below record the reconciliations *as performed at the 29/134/33 snapshot*; the
> authoritative current figures are 35 / 234 / 49 / 6 (see §1 and `formal/count-artifacts.sh`). Later passes
> took the suite 134→200→208→234 lemmas (NTT-inversion, Montgomery, the multi-level CT/CRT transform, the
> bit-reversal identity, and the literal `while`-loop NTT procs) and 33→45→47→49 genuineness checks.
...
| Formal verification | paper proofs (static/selective corruption) | paper proofs (ROM) | **234 machine-checked lemmas** (EasyCrypt+Coq) + 6 Gobra |
...
- **Source of truth** locked: `formal/count-artifacts.sh` → **35 artifacts / 234 lemmas (202 EC + 32 Coq) /
...
**CT-transcription pass (counts → 200 / 36-36; 35 artifacts).** The NTT-loop structural transcription is now
...
**CRT-split pass (counts → 164 / 37-37; 35 artifacts).** New `ml_adsa_ntt_crt.ec` adds the *stride-structure*
...
**Flat-array factor-tree pass (counts → 200 / 38-38; 35 artifacts).** `ml_adsa_ntt_crt.ec` is extended from
...
**NTT residuals (a)+(b) closed + norm-budget study (counts → 180 / 40-40; 35 artifacts).**
...
**NTT loop fully closed — termination + literal index arithmetic (counts → 200 / 42-42; 35 artifacts).**
