...
`formal/check-all.sh` → **25 classical EasyCrypt + 5 quantum (EasyPQC) + 5 Rocq = 35 artifacts**;
`formal/genuineness.sh` → **49/49** (each weakening breaks its proof). Go: `go test ./...` PASS.
...
theorem), `dkey_ll`. Verified: zero `admit/Admitted/sorry`; 49/49 genuineness.
