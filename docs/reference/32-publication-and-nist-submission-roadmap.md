...
| 3 | Machine-checked proofs (beyond required, a differentiator) | **DONE** | 244 lemmas (212 EC + 32 Coq) across 36 artifacts + 6 Gobra; `formal/count-artifacts.sh`, `docs/31`, `docs/35` |
...
| 6 | Optimized + constant-time implementation | **DONE (perf-measure pending)** | branchless **constant-time** reduction (`modQ`/`cabs`) + allocation-fused arithmetic applied & benchmarked (`docs/33 §4b`, `docs/34`); byte-identical. **AVX2 NTT/INTT** — full Montgomery-domain kernel (`ntt_amd64.s`), byte-identical to generic, differentially validated under docker linux/amd64 and **enabled by default on AVX2 CPUs**; only a native-x86 throughput measurement remains (`bench-avx2.sh`, `docs/34 §3`) |
...
| 8 | Multiple parameter sets (Cat 2/3/5) | **DONE** | -44/-65/-87 fully instantiated in go-mladsa: params.go (defs+sizes), verify_param.go (parameterized verifier), sign_param.go (KeyGenP+SignP), aggregate_param.go (multi-signer aggregate). Cross-validated against CIRCL mldsa44/65/87 — our base signatures AND our multi-signer aggregate (pk*,sig*) are accepted by CIRCL's independent verifier at all three sets; tamper rejected (param_test.go, sign_param_test.go, aggregate_param_test.go) |
