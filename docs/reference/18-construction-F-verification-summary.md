...
`formal/check-all.sh` → **21 classical EasyCrypt + 5 quantum (EasyPQC) + 5 Rocq = 31 artifacts**;
`formal/genuineness.sh` → **36/36** (each weakening breaks its proof). Go: `go test ./...` PASS.
...
theorem), `dkey_ll`. Verified: zero `admit/Admitted/sorry`; 36/36 genuineness.
