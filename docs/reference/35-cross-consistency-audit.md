...
Machine-checked lemmas : 191   (159 EasyCrypt + 32 Coq)
Genuineness checks : 42/42
...
The deprecated tallies seen in older prose — **77** (= 72 EC + 5 Coq), **42**, **45**, **23**
...
| Formal verification | paper proofs (static/selective corruption) | paper proofs (ROM) | **191 machine-checked lemmas** (EasyCrypt+Coq) + 6 Gobra |
...
- **Source of truth** locked: `formal/count-artifacts.sh` → **32 artifacts / 191 lemmas (159 EC + 32 Coq) /
  36-36** genuineness / 6 Gobra. Successive passes (§6): count-audit 29/134/33 → proof-closure 29/137/34 →
  encoding-conformance 30/153/35 (`ml_adsa_montgomery.ec`) → CT-transcription 31/191/36
...
- **Public site** re-synced via the new `ml-adsa-site/sync-docs.sh` (36 reference copies refreshed + docs/35
  added to nav; landing-page lemma count 77→191).
...
**CT-transcription pass (counts → 191 / 36-36; 32 artifacts).** The NTT-loop structural transcription is now
...
**CRT-split pass (counts → 164 / 37-37; 32 artifacts).** New `ml_adsa_ntt_crt.ec` adds the *stride-structure*
...
**Flat-array factor-tree pass (counts → 191 / 38-38; 32 artifacts).** `ml_adsa_ntt_crt.ec` is extended from
...
**NTT residuals (a)+(b) closed + norm-budget study (counts → 180 / 40-40; 32 artifacts).**
...
**NTT loop fully closed — termination + literal index arithmetic (counts → 191 / 42-42; 32 artifacts).**
